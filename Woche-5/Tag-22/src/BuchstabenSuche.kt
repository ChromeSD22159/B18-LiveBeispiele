...
fun main(){
    ...
    for (zeichen in songText.lowercase()){   // .lowercase um Groß-Kleinschreibung ignorieren
    ...
    // Ansatz mit .replace
    var anzahl = songText.length - songText.lowercase().replace(suchBuchstabe.toString(), "").length

    // .count
    anzahl = songText.lowercase().count { buchstabe ->
        buchstabe == suchBuchstabe
    }
