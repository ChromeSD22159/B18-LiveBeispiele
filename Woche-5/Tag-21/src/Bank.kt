...
/*
    ...
    //d.H hier können auch nur Funktionen aufgerufen werden.
    println(bank)
...
*/
fun kontoHinzufuegen(konto: String, betrag: Double = 0.0): Boolean {
...
fun kontoLoeschen(konto: String) {
    if (konto !in bank.keys) { // bank.containsKey(konto)
        //konto existiert noch nicht
...
fun geldAbheben(konto: String, betrag: Double) {
    ...
    if (bank.containsKey(konto)) { // konto in bank
        ...
        if (betrag > 500.0){
            ...
            println("Auszahlung fehlgeschlagen. Nicht ausreichender Kontostand!")
        } else
        {
            //Auszahlung möglich
            bank[konto] = kontoStand - betrag

            // bank[konto] -= betrag    -> nicht möglich, weil Abruf in Map nullable
