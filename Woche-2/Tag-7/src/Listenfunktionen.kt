...
fun main(){
    ...
    var minArtikel: String = artikelListe[gesuchtePosition]

    println("Das günstigste Produkt ist $minArtikel (kostet $minPreis€)")

    val maxArtikel = artikelListe[preisListe.indexOf(preisListe.max())]
    println("Das teuerste Produkt ist $maxArtikel (kostet ${preisListe.max()}")

    var durchschnittsPreis = preisListe.average()
    durchschnittsPreis = preisListe.sum() / preisListe.size
