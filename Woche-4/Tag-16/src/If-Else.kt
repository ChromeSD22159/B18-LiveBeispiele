fun main() {
    ...
    val nutzerEingeloggt: Boolean = listOf(true, false).random()

    if (nutzerEingeloggt){
        val istVolljaehrig: Boolean = listOf(true, false).random()
        if (istVolljaehrig){
            println("Alle Produkte")
        }
        else {
            println("Kinder Produkt")
        }
    }
    else {
        println("Login Bildschirm")
    }
